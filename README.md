# 🎨 Colour Name Converter (CNC)

> A Python tool and Web Application that converts standard hex codes into colour names vice versa, using a comprehensive CSV database.

📹 **Video Demo:** [Watch on YouTube](https://www.youtube.com/shorts/0B-R-BhlIv0)

## 📖 Description

The CNC (Colour Name Converter) is a program designed to bridge the gap between technical hex codes and human-readable colour names. It provides two interfaces: a command-line script and a modern Streamlit web application.
...
The program uses a CSV file named `colors.csv` containing over 1,000 colours. If an input is not found in the database, the CNC informs the user. If the input format is incorrect, the program identifies it as an invalid name or code.
...
The Streamlit app (`cncapp.py`) adds several user-friendly features:
  ...
  * **Real-time Colour Preview:** Displays a visual block of the colour being searched.
  * **RGB Conversion:** Automatically calculates and shows the RGB values alongside the hex code.
  * **Interactive Sidebar:** Includes mode selection (Code to Name or Name to Code) and a "Random Colour" generator for discovery.
  * **Optimised Performance:** Uses `@st.cache_data` to ensure the colour database loads instantly after the first use.
...
### 4\. `convert_code(code, colours_dict)`
...
Ensures a colour name contains only allowed characters.

  * **Pattern:** `[a-zA-Z\s'’]+`
  * Allows letters, spaces, and both straight and curly apostrophes.

### 6\. `convert_name(name, colours_dict)`
...
The programme includes `colour.replace("’", "'")`. This is crucial because users might type names using curly apostrophes (common on mobile or Mac), while the CSV uses straight apostrophes. This normalisation ensures the search doesn't fail due to punctuation style.
...
The project uses `pytest` to ensure all core functions work reliably.
  ...
  * **`test_load_colours`**: Confirms the CSV loads and correctly maps "Yellow" to `#FFFF00`.
