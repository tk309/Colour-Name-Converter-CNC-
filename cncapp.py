...
# JavaScript to blur (hide keyboard) on button click
st.markdown("""
<script>
...
// Listen for all button clicks (Streamlit buttons have class 'stButton')
...
# ---------- TAB 1: Color name to hex code ----------
with tab1:
    st.subheader("Enter a color name")
    # Use a form so that the conversion only happens on button click
    with st.form(key="name_to_hex_form"):
        color_name_input = st.text_input(
            "Color Name:",
            placeholder="e.g., Red, Midnight Blue, Crimson",
            key="name_input"
        )
        submitted_name = st.form_submit_button("🔄 Convert to Hex Code")
        
        if submitted_name and color_name_input:
        ...
        elif submitted_name and not color_name_input:
            st.warning("Please enter a color name.")

# ---------- TAB 2: Hex code to color name ----------
with tab2:
    st.subheader("Enter a hex color code")
    with st.form(key="hex_to_name_form"):
        hex_input_raw = st.text_input(
            "Hex Code:",
            placeholder="e.g., #FF0000, #FFFFFF, #663399",
            key="hex_input"
        )
        submitted_hex = st.form_submit_button("🔄 Convert to Color Name")
        
        if submitted_hex and hex_input_raw:
        ...
        elif submitted_hex and not hex_input_raw:
